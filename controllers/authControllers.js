...
const signUp = async (req, res) => {
  try {
    ...
    // Hash the password
    const hashedPassword = await hash(password, 20); // You can adjust the salt rounds as needed

    // Create a new user
    const newUser = new User({username,email,password:hashedPassword});//password to be hashed
    await newUser.save();
...
const login = async (req, res) => {
  try{
    ...
    //compare hashed passw if correct
